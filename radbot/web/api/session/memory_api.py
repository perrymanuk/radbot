    ...
    try:
        ...
        if hasattr(runner, "runner"):
            ...
            if hasattr(runner.runner, "agent"):
                logger.info(
                    f"Runner.runner.agent type: {type(runner.runner.agent).__name__}"
                )
        ...
        # Next check if the runner.runner has it (from the Runner object)
            ...
            hasattr(runner, "runner")
            and hasattr(runner.runner, "memory_service")
            and runner.runner.memory_service
        ):
        ...
        if not memory_service:
            try:
                ...
                logger.info(
                    "Created QdrantMemoryService on demand for memory storage API"
                )
        ...
        if not memory_service:
            raise HTTPException(
                status_code=500,
                detail="Memory service not available - tried all fallback methods",
            )
        ...
        point = memory_service._create_memory_point(
            user_id=user_id, text=request.text, metadata=metadata
        )
