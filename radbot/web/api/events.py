...
def add_event(session_id: str, event: Dict[str, Any]) -> None:
    ...
    for existing_event in session_events[session_id]:
        ...
        if (
            existing_event.get("type") == event.get("type")
            and existing_event.get("summary") == event.get("summary")
            and existing_event.get("timestamp") == event.get("timestamp")
        ):
            is_duplicate = True
            logger.debug(
                f"Skipping duplicate event: {event.get('type')} - {event.get('summary')}"
            )
            break
