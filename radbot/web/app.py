...
async def initialize_app_startup():
    ...
    try:
        ...
        _qdrant_coll = (
            _cl.get_config().get("vector_db", {}).get("collection", "radbot_memories")
        )
        logger.info("=" * 60)
        ...
        try:
            ...
            enabled_server_ids = {s.get("id") for s in _cl.get_enabled_mcp_servers()}
            all_server_ids = {s.get("id") for s in _cl.get_mcp_servers()}
            ...
            if disabled_server_ids:
                ...
                try:
                    ...
                    for sid in disabled_server_ids:
                        client = (
                            MCPClientFactory.get_client(sid)
                            if MCPClientFactory
                            else None
                        )
                        if client:
                            server_tools = (
                                client.tools if hasattr(client, "tools") else []
                            )
        ...
        try:
            ...
            if stt_config.get("enabled", True):
                STTService.create_instance(
                    ...
                    enable_automatic_punctuation=stt_config.get(
...
class ConnectionManager:
    ...
    async def send_message(self, session_id: str, message: str):
        await self._send_to_all(session_id, {"type": "message", "content": message})

    async def send_status(self, session_id: str, status: str):
        await self._send_to_all(session_id, {"type": "status", "content": status})
    ...
    async def send_events(self, session_id: str, events: list):
        ...
        if len(events_json) > max_size:
            ...
            logger.warning(
                f"Event payload too large: {len(events_json)} bytes. Splitting into chunks."
            )
            ...
            for event in events:
                ...
                if event_size > max_size:
                    ...
                    logger.warning(
                        f"Oversized event: {event_type} - {event_summary}: {event_size} bytes"
                    )
                    if (
                        "text" in event
                        and isinstance(event["text"], str)
                        and len(event["text"]) > 100000
                    ):
                        ...
                        await self._send_to_all(
                            session_id, {"type": "events", "content": [event]}
                        )
                else:
            ...
            await self._send_to_all(session_id, {"type": "events", "content": events})
    ...
    try:
                ...
                if hasattr(event, "content") and event.content:
                    ...
                    if text_content:
                        message = {
                            ...
                            "timestamp": int(
                                getattr(event, "timestamp", time.time()) * 1000
                            ),  # Convert to JS timestamp
                            "agent": getattr(event, "agent_name", None)
                            or getattr(event, "agent", None),
                        }
        ...
        async def handle_sync_request(last_message_id, timestamp=None):
            ...
            app_name = (
                ...
                if hasattr(runner, "runner") and hasattr(runner.runner, "app_name")
                else "beto"
            )

            # Retrieve the session from ADK (get_session is async in ADK 1.21.0)
            session = await runner.session_service.get_session(
                app_name=app_name, user_id=runner.user_id, session_id=session_id
            )

            if not session:
                logger.warning(
                    f"No session found for sync request with ID {session_id}"
                )
                await websocket.send_json({"type": "sync_response", "messages": []})
                return
        ...
        async def handle_history_request(limit=50):
            ...
            try:
                ...
                logger.warning(
                    f"Failed to load history from DB, falling back to ADK session: {db_err}"
                )
            ...
            if not session:
                logger.warning(
                    f"No session found for history request with ID {session_id}"
                )
                await websocket.send_json(
                    {"type": "history", "session_id": session_id, "messages": []}
                )
                return
            ...
            if data.get("type") == "heartbeat":
                ...
                if last_message_id:
                    ...
                    await manager.send_status(
                        session_id, "error: Missing lastMessageId in sync_request"
                    )
                continue
            ...
            try:
                ...
                if target_agent:
                    ...
                    if target_agent.lower() == "scout":
                        ...
                        for sub_agent in root_agent.sub_agents:
                                ...
                                try:
                                    ...
                                    if hasattr(
                                        ...
                                        response_text = process_request(
                                            sub_agent, user_message
                                        )
                                except Exception as e:
                                    logger.error(
                                        f"Error with direct generation: {str(e)}",
                                        exc_info=True,
                                    )
                            ...
                            logger.warning(
                                "Could not find Scout agent directly in sub_agents"
                            )
                            ...
                            if target:
                                ...
                                logger.warning(
                                    f"Target agent {target_agent} not found, using default runner"
                                )
                ...
                try:
                    ...
                    chat_operations.add_message(
                        session_id, "user", user_message, user_id="web_user"
                    )
                    if response:
                        chat_operations.add_message(
                ...
                logger.error(
                    f"Error processing WebSocket message: {str(e)}", exc_info=True
                )
