...
export function TavilyPanel() {
  const { loadLiveConfig, saveCredential, testConnection, toast, loadStatus, status } =
    useAdminStore();
  ...
  useEffect(() => {
    loadLiveConfig().then((cfg) => {
      const tavily = dig(cfg, "integrations.tavily", {});
      if (tavily.api_key) setApiKey(MASKED);
    });
  }, [loadLiveConfig]);
  ...
  const handleTest = async () => {
    ...
    try {
      ...
      const result = await testConnection("tavily", body);
      setTestResult(result);
    } catch (e: any) {
      setTestResult({ status: "error", message: e.message });
    } finally {
  ...
  const handleSave = async () => {
    ...
    try {
      if (apiKey && apiKey !== MASKED) {
        await saveCredential("tavily_api_key", apiKey, "api_key", "Tavily API key");
      }
  ...
  const tavilyStatus = status?.tavily?.status || "unconfigured";

  return (
    <div>
      <div className="flex items-center gap-2.5 mb-4">
        <h2 className="text-lg font-semibold text-[#eee]">Tavily</h2>
...
export function Crawl4aiPanel() {
  ...
  useEffect(() => {
    loadLiveConfig().then((cfg) => {
      const c4a = dig(cfg, "integrations.crawl4ai", {});
      setEnabled(!!c4a.enabled);
      setApiUrl(c4a.api_url || "");
      setCollection(c4a.collection || "");
      if (c4a.api_token) setApiToken(MASKED);
    });
  }, [loadLiveConfig]);
  ...
  const handleTest = async () => {
    ...
    try {
      ...
      if (apiToken && apiToken !== MASKED) body.api_token = apiToken;
      const result = await testConnection("crawl4ai", body);
      setTestResult(result);
    } catch (e: any) {
      setTestResult({ status: "error", message: e.message });
    } finally {
  ...
  const handleSave = async () => {
    ...
    try {
      if (apiToken && apiToken !== MASKED) {
        await saveCredential("crawl4ai_api_token", apiToken, "api_key", "Crawl4ai API token");
      }
      await mergeConfigSection("integrations", {
        crawl4ai: {
          enabled,
  ...
  const crawl4aiStatus = status?.crawl4ai?.status || "unconfigured";

  return (
    <div>
      <div className="flex items-center gap-2.5 mb-4">
        <h2 className="text-lg font-semibold text-[#eee]">Crawl4ai</h2>
        <StatusBadge status={crawl4aiStatus} />
      </div>
