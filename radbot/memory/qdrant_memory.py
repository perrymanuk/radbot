...
class QdrantMemoryService(BaseMemoryService):
        ...
        try:
            ...
            # Second priority: Use provided URL (with or without API key)
                ...
                if api_key:
                    ...
                    self.client = QdrantClient(
                        url=url, https=use_https, prefer_grpc=False
                    )
                    logger.info(
                        f"Connected to Qdrant at {url} ({'HTTPS' if use_https else 'HTTP'} mode, gRPC disabled)"
                    )
    ...
    def _initialize_collection(self):
        ...
        while retry_count < max_retries:
            try:
                ...
                logger.info(
                    f"Created Qdrant collection '{self.collection_name}' with vector size {self.vector_size}"
                )
                ...
                retry_count += 1
                logger.warning(
                    f"Qdrant operation attempt {retry_count} failed: {str(e)}"
                )
        ...
        logger.error(
            f"Failed to initialize Qdrant collection after {max_retries} attempts: {str(last_error)}"
        )
        raise last_error
    ...
    def add_session_to_memory(self, session: Session) -> None:
        ...
        try:
            ...
            if not session.events:
                logger.info(
                    f"No events found in session {session.id}, skipping memory ingestion"
                )
                return
            ...
            if not points:
                logger.info(
                    f"No valid text events found in session {session.id}, skipping memory ingestion"
                )
                return
            ...
            logger.info(
                f"Successfully added {len(points)} memory points from session {session.id}"
            )
    ...
    def _create_memory_point(
        ...
        vector = embed_text(
            text, self.embedding_model, is_query=False, source="agent_memory"
        )
    ...
    def search_memory(
        ...
        try:
            ...
            query_vector = embed_text(
                query, self.embedding_model, is_query=True, source="agent_memory"
            )
            ...
            if filter_conditions:
                    ...
                    must_conditions.append(
                        models.FieldCondition(
                            ...
                            match=models.MatchValue(
                                value=filter_conditions["source_agent"]
                            ),
                        )
                    )
                ...
                if "memory_type" in filter_conditions:
                    must_conditions.append(
                        models.FieldCondition(
                            key="memory_type",
                            match=models.MatchValue(
                                value=filter_conditions["memory_type"]
                            ),
                        )
                    )
                ...
                if "min_timestamp" in filter_conditions:
                    must_conditions.append(
                        models.FieldCondition(
                            key="timestamp",
                            range=models.Range(gte=filter_conditions["min_timestamp"]),
                        )
                    )
                ...
                if "max_timestamp" in filter_conditions:
                    must_conditions.append(
                        models.FieldCondition(
                            key="timestamp",
                            range=models.Range(lte=filter_conditions["max_timestamp"]),
                        )
                    )
            ...
            for result in query_response.points:
                ...
                for key, value in payload.items():
                    if (
                        key not in entry and key != "user_id"
                    ):  # Skip user_id and already added fields
                        entry[key] = value
