...
async def _create_home_assistant_toolset_async() -> (
    ...
    try:
        ...
        if not ha_mcp_url:
            logger.error(
                "Home Assistant MCP URL not found. "
        ...
        if not ha_auth_token:
            logger.error(
                "Home Assistant authentication token not found. "
        ...
        if not ha_mcp_url.endswith("/mcp_server/sse") and not ha_mcp_url.endswith(
            "/api/mcp_server/sse"
        ):
            ...
            logger.info(
                f"Normalized Home Assistant MCP URL from {original_url} to {ha_mcp_url}"
            )
        ...
        ha_mcp_params = SseServerParams(
            url=ha_mcp_url, headers={"Authorization": f"Bearer {ha_auth_token}"}
        )
        ...
        try:
            tools, _ = await McpToolset.from_server(
                connection_params=ha_mcp_params, async_exit_stack=exit_stack
            )
    ...
    try:
        ...
        try:
            ...
            if existing_loop.is_running():
                logger.warning(
                    "Cannot create Home Assistant toolset: Event loop is already running"
                )
                logger.warning(
                    "This likely means you're using this in an async context"
                )
                logger.warning(
                    "Try using 'await _create_home_assistant_toolset_async()' instead"
                )
                return []
        except RuntimeError:
        ...
        tools, exit_stack = loop.run_until_complete(
            _create_home_assistant_toolset_async()
        )
