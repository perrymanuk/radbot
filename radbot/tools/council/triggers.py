...
_RISK_KEYWORDS = {
    "auth": (
        "auth", "oauth", "token", "session", "credential", "secret",
        "password", "api key", "api_key", "permission", "authz", "authn",
    ),
    "integration": (
        "new integration", "new dependency", "new api", "new client",
        "new mcp server", "webhook", "third-party",
    ),
    "storage": (
        "schema", "migration", "add column", "alter table", "create table",
        "drop table", "new table", "rename column", "backfill",
    ),
    "deploy": (
        "nomad", "docker", "ci/cd", "pipeline", "rollout", "deploy",
        "release", "ship", "production", "irreversible",
    ),
    "security": (
        "vulnerability", "cve", "sanitize", "prompt injection",
        "cross-site", "xss", "csrf", "ssrf", "rce",
    ),
}
