...
| **scout** | `agent/research_agent/factory.py` | 2 memory + 3 wiki + 2 web_research (grounded_search + web_fetch) + 11 telos (scoped subset) + 5 council (4 critics + trigger) | Research + planning: writes plans to Telos (explorations + project_tasks) for Claude Code to pick up via MCP. Runs plans through a 3-round persona council before persisting — see `specs/agents.md` § Scout Plan Council. Selectable as session root (alongside beto) — `chat_sessions.agent_name`. |
