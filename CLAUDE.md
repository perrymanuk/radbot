...
| **scout** | `agent/research_agent/factory.py` | 2 memory + 3 wiki + 2 web_research + 11 telos (scoped subset) + 5 council + 5 repo_exploration + 1 divergent_ideation + 3 claude_code_session (start/poll/reply) | Research + planning: writes plans to Telos (explorations + project_tasks) for Claude Code to pick up via MCP. Also kicks off Claude Code sessions directly via async start/poll/reply wrapping the existing `ClaudeCodeClient` (EX26/PT77). Runs plans through a 3-round persona council before persisting — see `specs/agents.md` § Scout Plan Council. Selectable as session root (alongside beto) — `chat_sessions.agent_name`. |
