...
| **scout** | `agent/research_agent/factory.py` | 2 memory + 3 wiki + 1 web_fetch + 11 telos (scoped subset) | Research + planning: writes plans to Telos (explorations + project_tasks) for Claude Code to pick up via MCP. Selectable as session root (alongside beto) — `chat_sessions.agent_name`. |
