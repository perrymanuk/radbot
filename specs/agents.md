...
| **scout** | `agent/research_agent/factory.py` | `config_manager.get_agent_model("scout_agent")` (`gemini-3.1-pro-preview`) | 18 | Research + planning; writes plans to Telos (exploration + project_tasks). Selectable as session root (see "Session Roots" below). |
...
| Divergent Ideation | 1 | `radbot.tools.divergent_ideation.execute_divergent_ideation_tool` — fires 3 parallel LLM streams (standard / contrarian / wildcard) via the shared genai client with `asyncio.gather(return_exceptions=True)`. Returns `{status, standard, contrarian, wildcard, ...}`; failed streams degrade to inline error dicts so one stream failure does not sink the batch. |
