...
| **scout** | `agent/research_agent/factory.py` | `config_manager.get_agent_model("scout_agent")` (`gemini-3.1-pro-preview`) | 34 | Research + planning; writes plans to Telos (exploration + project_tasks). Selectable as session root (see "Session Roots" below). |
...
| Claude Code session (async) | 3 | `radbot.tools.claude_code_session.CLAUDE_SESSION_TOOLS` — `start_claude_session`, `poll_claude_session`, `reply_claude_session`. Wraps `ClaudeCodeClient.start_background_session` (subprocess with `stdin=PIPE` + `--dangerously-skip-permissions`, stream-json reader task). Does **not** use `claude-agent-sdk` (which requires raw `ANTHROPIC_API_KEY`); reuses RadBot's custom `CLAUDE_CODE_OAUTH_TOKEN` flow. See EX26 / PT77 in Telos. |
