...
- **Before-model callbacks**: `[scrub_empty_content_before_model, sanitize_before_model_callback, sanitize_tool_schemas_before_model]` (no `inject_telos_context` — retired from beto by Item 6 / 2026-05-03; user-context paragraph moved into `main_agent.md` § Telos)
...
- **Telos persona injection** (scout-as-root only): `inject_telos_context` appends an anchor (~300B: identity + mission + counts + tool pointer) to `llm_request.config.system_instruction` on every turn. On the first turn of each session it also appends the full block (~2KB: mission, problems, goals, active projects, challenges, wisdom, last 5 journal entries), gated by `callback_context.state["telos_bootstrapped"]`. **Beto previously received this too** (line 317 of `assembly.py` pre-Item-6) but was retired because chitchat / pure-routing turns paid the token tax for context they never used; beto now relies on the static persona paragraph in `main_agent.md` plus on-demand `telos_get_section/entry/full` tool calls. Sub-agents are tool executors and do **not** receive Telos context. See `docs/implementation/telos.md` and `radbot/agent/research_agent/factory.py:217-242` (scout-as-root callback registration site) + `radbot/agent/assembly.py` (gating site that decides whether scout is built `as_root`).
...
| Claude Code session (async) | 5 | `radbot.tools.claude_code_session.CLAUDE_SESSION_TOOLS` — `start_claude_session(task_ref, target_dir, auto_ship?, resume_session_id?, inject_github_token?)`, `run_claude_session(task_ref, target_dir, auto_ship?, timeout?)` (bounded-sync, 90s cap), `poll_claude_session`, `reply_claude_session`, `wait_claude_session(timeout_seconds?, hard cap 600s)`. **Strict Pydantic boundary (EX35 / PT96):** kickoff tools (`start_claude_session` + `run_claude_session`) accept only a `task_ref` matching `^(PT\|EX)\d+$` — no raw `prompt` / `custom_prompt` parameter exists. The prompt is synthesized internally as `execute <ref>` (with `and use /ship when complete` appended when `auto_ship=True`); `ValidationError` from `ClaudeSessionRequest` is caught and returned as `{status: "error", message: ...}` so the agent loop survives. `inject_github_token=True` injects a GitHub App installation token as `GH_TOKEN`/`GITHUB_TOKEN` into the subprocess env so `gh` can push and open PRs without interactive auth prompts. `wait_claude_session` blocks (sleeping 5s between polls) until `job_status` leaves `running`. Wraps `ClaudeCodeClient.start_background_session` (subprocess with `stdin=PIPE` + `--dangerously-skip-permissions`, stream-json reader task). Does **not** use `claude-agent-sdk` (which requires raw `ANTHROPIC_API_KEY`); reuses RadBot's custom `CLAUDE_CODE_OAUTH_TOKEN` flow. See EX26 / PT77 / EX30 / PT85 / EX35 / PT96 in Telos. |
...
- **Telos persona**: scout-as-root is the **sole** consumer of `inject_telos_context` (plans must be grounded in identity/mission/goals). The sub-agent scout instance does not get it; beto does not get it either (Item 6 / 2026-05-03 retirement). Registered at `radbot/agent/research_agent/factory.py:217-242` (the `if as_root:` block).
...
**Postmortem processing flow** (Scout instruction file § 7 — never autonomous; user-triggered only). Scout queries `journal` rows where `metadata.type="postmortem" AND metadata.processed_at IS NULL`, parses each one's `## Followups` section into the three sub-sections (`### Tasks` → `task`, `### Explorations` → `exploration`, `### Insights` → `journal_insight`), derives a deterministic dedup key as `sha256(f"{jr_ref}|{role}|{ordinal}").hexdigest()[:16]` (ordinal from the markdown's numbered list, NOT LLM whim), then atomically `*_add`s each followup with `metadata_merge={source_postmortem, postmortem_followup_role, postmortem_followup_key}` after a dedup query on the same triple. Marks the postmortem `processed_at` only after every followup for it succeeded, with `followup_refs` as the union of dedup-hit and newly-created refs. The `## Followups` markdown contract is the input/output handshake between `/postmortem-ex` (Item 4 — writes it) and Scout (reads it); `_None_` is the sentinel for empty sub-sections.
...
| `inject_telos_context` | `tools/telos/callback.py` | scout-as-root only (before_model) | Inject Telos anchor every turn + full block on first turn of session into `system_instruction`. Registered at `research_agent/factory.py:217-242`. **Retired from beto** by Item 6 / 2026-05-03 — beto now relies on the static persona paragraph in `main_agent.md` § Telos plus on-demand `telos_get_section/entry/full` tool calls. |
| `handle_empty_response_after_model` | `callbacks/empty_content_callback.py` | all (after_model) | Nullify `llm_response.content` on `Content(parts=None)` so ADK's `base_llm_flow.py:924-931` skips appending the empty model event to `session.events`. The session runner then sees no text, walks events for a non-transfer `function_response`, and either (a) retries on the same session with a continuation prompt that embeds the original user request, or (b) resets the session and replays the original. See `web/api/session/session_runner.py::_run_with_empty_content_retry`. |
