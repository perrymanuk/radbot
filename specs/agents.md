...
| **scout** | `agent/research_agent/factory.py` | `config_manager.get_agent_model("scout_agent")` (`gemini-3.1-pro-preview`) | 32 | Research + planning; writes plans to Telos (exploration + project_tasks). Selectable as session root (see "Session Roots" below). |
