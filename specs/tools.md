...
When a scheduled task fires, `SchedulerEngine._execute_job` runs the prompt through the dedicated session and emits a `ScheduledTaskEvent` to the Notifier seam (`radbot/services/notifier.py`); the Notifier owns the result fan-out across chat history, ntfy, the `notifications` table, and WS notification badges. Reminder firings (`_execute_reminder`) and late deliveries (`_deliver_single_reminder`) emit `ReminderEvent` to the same seam — see `SPEC.md` § Notifier seam.
