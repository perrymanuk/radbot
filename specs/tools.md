...
| `tools/divergent_ideation/` | 1 | scout | `execute_divergent_ideation(problem_statement)` — 3 parallel LLM streams (standard / contrarian / wildcard) via `create_client_with_config_settings()` (shared genai client). Uses `asyncio.gather(..., return_exceptions=True)`; failed streams return as inline `{error, message}` dicts. Returns `{status, problem_statement, model, standard, contrarian, wildcard}`. Per-stream structured logs capture latency, tokens, status. |
