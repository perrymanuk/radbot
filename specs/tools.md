...
When a scheduled task fires, `SchedulerEngine._execute_job` runs the prompt through the dedicated session and emits a `ScheduledTaskEvent` to the Notifier seam (`radbot/services/notifier.py`); the Notifier owns the result fan-out across chat history, ntfy, the `notifications` table, and WS notification badges. Reminder firings (`_execute_reminder`) and late deliveries (`_deliver_single_reminder`) emit `ReminderEvent` to the same seam. Alertmanager pipeline (`tools/alertmanager/processor.py`) emits `AlertEvent` (with `phase` discriminator) at each remediation step, and the Heartbeat digest (`tools/heartbeat/delivery.py`) emits `HeartbeatEvent` — both fan out through the same seam — see `SPEC.md` § Notifier seam.
