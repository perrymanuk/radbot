...
| `tools/claude_code_session/` | 4 | scout | `start_claude_session(prompt, target_dir, resume_session_id?, inject_github_token?)` → `{job_id}`; `poll_claude_session(job_id)` → `{job_status, output (delta), waiting_for_input, pending_question, session_id, return_code, stderr}`; `reply_claude_session(job_id, reply)` writes to the CLI's stdin and flushes; `wait_claude_session(job_id, timeout_seconds?)` blocks (sleeping 5s between polls, hard cap 600s) until `job_status` leaves `running`, returning the same dict as `poll_claude_session` plus `timed_out`. `inject_github_token=True` on `start_claude_session` fetches a GitHub App installation token via `GitHubAppClient._get_installation_token()` and injects it as `GH_TOKEN`/`GITHUB_TOKEN` into the subprocess env so `gh` can push branches and open PRs — token is never logged. Wraps `ClaudeCodeClient.start_background_session` — a subprocess launched with `stdin=asyncio.subprocess.PIPE`, `--dangerously-skip-permissions`, and a reader task parsing the `stream-json` stdout. We deliberately do **not** use `claude-agent-sdk` (requires `ANTHROPIC_API_KEY`; RadBot uses the custom `CLAUDE_CODE_OAUTH_TOKEN` flow already wired through `ClaudeCodeClient`). `waiting_for_input` is set when the stream emits `permission_request` / `input_request` / `ask_user_question` events or an `AskUserQuestion`-named `tool_use` block; Scout replies via `reply_claude_session` to resume. Session state (buffer + proc handle) is process-local; restarts drop in-flight jobs. See EX26 / PT77 / EX30 / PT85. |
