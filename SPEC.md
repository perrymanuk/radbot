...
- **Notifier seam** (`radbot/services/notifier.py`): single fan-out module that takes a polymorphic `ResultEvent` (subclasses: `ScheduledTaskEvent`, `ReminderEvent`, `AlertEvent`, `HeartbeatEvent`) and dispatches it to the canonical sinks — chat history DB, ntfy, notifications row, WebSocket chat, alert_result WS broadcast — concurrently via `asyncio.gather(return_exceptions=True)` with per-sink `asyncio.wait_for` timeouts. Sinks are constructor-injected (`NotificationSink` Protocol) so tests substitute an in-memory fake. `WsNotificationPayload` / `WsSystemMessagePayload` Pydantic models validate the WS broadcast shapes. `AlertEvent.phase` (`received` / `investigating` / `resolved` / `failed`) is a sink-inspected discriminator: sinks return early for phases they don't handle (no `skip_sinks` routing for alerts). Per-sink exceptions and timeouts log at `ERROR` level so dropped events are noisy. PR1 (EX41) migrated scheduler + reminders; PR2 (EX42 / PT106) migrated alertmanager + heartbeat; PR3 will migrate webhooks. See `docs/plans/EX_DRAFT_notifier_seam.md`.
- **Projects viewer**: `/projects` (and `/projects/:refCode?tab=`) renders the Telos project hierarchy — projects → milestones → tasks (bucketed by `metadata.task_status`) + explorations + goals. Two-pane: filterable list rail + tabbed detail. Read-only v1. Backed by two unauth'd reads (`/api/telos/projects/summary`, `/api/telos/projects/entries`); parent linkage resolved client-side via `metadata.parent_project` / `metadata.parent_milestone`. Normalized Zustand store keyed by `section:ref_code`. See `specs/web.md` § Projects Page.
...
- **`radbot/services/`**: Cross-cutting service modules that sit above `tools/` and below `web/api/` — code that isn't a FunctionTool but also isn't a REST route. Current contents: `session_naming.py` (auto-name a chat session from recent history via a cheap LLM call; used by the `/name` intercept in `web/api/messages.py` and the CLI `/name` command — model resolution honors `RADBOT_NAMING_MODEL` env var, then `config:agent.agent_models.naming_model`, then falls back to `main_model`; 5s LLM timeout enforced via `ThreadPoolExecutor`); `notifier.py` (Notifier seam — see "Notifier seam" cross-cutting bullet above).
