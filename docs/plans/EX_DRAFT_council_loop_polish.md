...
**Still open (per Implementation order — see end of doc):** Item 1.b + 1.c (docs into `specs/agents.md`, `scout.md`, `SKILL.md`, `CLAUDE.md`); Item 3 (Scout instruction edits for postmortem processing); Item 6 (`inject_telos_context` removal from beto + scout-as-root verification); Item 2 (`/review-ex` skill — sibling commit in `~/git/perrymanuk/claude-skills`); Item 4 (`/postmortem-ex` skill); Item 5 (followup conventions formalization — paperwork once Items 3+4 land). The lifecycle ref-code flip from `EX_DRAFT_*` to a Telos `EX<N>` happens as part of Item 1.b/1.c.
