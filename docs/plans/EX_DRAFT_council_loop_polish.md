...
**Still open (per Implementation order — see end of doc):** Item 3 (Scout instruction edits for postmortem processing — next up); Item 6 (`inject_telos_context` removal from beto + scout-as-root verification); Item 2 (`/review-ex` skill — sibling commit in `~/git/perrymanuk/claude-skills`); Item 4 (`/postmortem-ex` skill); Item 5 (followup conventions formalization — paperwork once Items 3+4 land). The lifecycle ref-code flip from `EX_DRAFT_*` to a Telos `EX<N>` was deferred from PR #114 (creating the Telos row first would have widened the PR scope past pure docs); fold into Item 3's PR.
