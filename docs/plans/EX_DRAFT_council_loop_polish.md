...
**Still open (per Implementation order — see end of doc):** Item 6 (`inject_telos_context` removal from beto + compensating `main_agent.md` persona paragraph + scout-as-root verification — next up); Item 2 (`/review-ex` skill — sibling commit in `~/git/perrymanuk/claude-skills`); Item 4 (`/postmortem-ex` skill); Item 5 (followup conventions formalization — paperwork once Items 4 lands; Item 3's followup contract is already in scout.md). The lifecycle ref-code flip from `EX_DRAFT_*` to a Telos `EX<N>` is still deferred — the spec body is 105KB, would dominate any PR diff, and on-disk markdown remains useful for git-diff-ing shipping status updates. Will be its own small operation when there's a clean reason to do it.
