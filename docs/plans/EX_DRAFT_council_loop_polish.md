...
**Still open (per Implementation order — see end of doc):** Item 4 (`/postmortem-ex` sibling skill — `~/git/perrymanuk/claude-skills`, next up); Item 5 (followup conventions formalization — paperwork once Item 4 lands; Item 3's followup contract is already in scout.md). All radbot-side work is SHIPPED (Items 0/1.a/1.b/1.c/3/6 across PRs #113/#114/#115/#116). All council-loop wiring is operational end-to-end except postmortem-write — Scout already knows how to PROCESS postmortems (Item 3), but `/postmortem-ex` (Item 4) is what creates them after `/ship` returns. The lifecycle ref-code flip from `EX_DRAFT_*` to a Telos `EX<N>` is still deferred — the spec body is 105KB, would dominate any PR diff, and on-disk markdown remains useful for git-diff-ing shipping status updates. Will be its own small operation when there's a clean reason to do it.
