...
**Item 4 + Item 5 SHIPPED** in [claude-skills commit `4938d34`](https://github.com/perrymanuk/claude-skills/commit/4938d34), pushed direct to `claude-skills:main` 2026-05-03 (skill-only changes; no CI/PR convention in that repo). New `plugins/radbot/commands/postmortem-ex.md` (365 LoC) implements the 8-step `/postmortem-ex <PR#>` orchestration: PR-URL canonicalization (scheme/host/lowercase/integer-num — byte-exact match required for JSONB `@>` to hit), case-insensitive path-aware EX resolver `(?i)(?:^|/)ex(\d+)\b` over branch → title → body with conflict abort (no silent first-wins), step-ordering revised per Round 2 (validate markers BEFORE `journal_add` so a malformed-marker abort never leaves an orphan JR), dedup query with `include_inactive: true` so archived JRs from prior `--supersede` runs still surface, `--supersede` delayed-archive (steps 4-7 normally, ONLY archive on step-7 success — closes Round 3 archive-before-create blocker), `--force-new` mark-old-processed-FIRST (invariant: Scout never sees two unprocessed JRs for the same PR — closes Round 3 force-new blocker), `## Followups` markdown contract (### Tasks / ### Explorations / ### Insights, numbered lists, `_None_` sentinel) explicitly load-bearing for Scout's deterministic dedup-key derivation, EX status preserved (single-ownership rule), final UX nudge tells the user to run "Scout, process postmortems" next. Item 5 is effectively shipped end-to-end now: Scout already had the consumer contract from PR #115; this commit ships the producer side. Marketplace description updated.

**Item 2 SHIPPED** in [claude-skills commit `62a7ddd`](https://github.com/perrymanuk/claude-skills/commit/62a7ddd), pushed direct to `claude-skills:main` 2026-05-03 (no CI / no PR — that repo's convention for skill-only changes). Two artifacts in one commit because they share a contract: (a) new `plugins/radbot/commands/review-ex.md` — the 7-step `/review-ex <EX_ref>` orchestration covering MCP reachability probe, `format=json` body fetch, marker-balance validation BEFORE-and-AFTER strip (closes Round 2 council "no change after sub" miss case), `in_review` status flip, `/mm-council:evaluate` invocation, `COUNCIL_VERDICT:` line parse, synthesis-file read with absolute-path validation, content append with cross-family block always normalized to end-of-body, single `AskUserQuestion` confirmation for the `approved` flip on `proceed`/`proceed_with_changes`, leave at `in_review` on `revise`/`reject`. Recovery semantic pinned: never roll back to `proposed` on partial failure. (b) `plugins/mm-council/commands/evaluate.md` step 6 (sibling change) — emits `COUNCIL_VERDICT: {verdict, confidence, panelist_count, synthesis_markdown_path}` as the **last** stdout line, AND writes the synthesis markdown to `${XDG_RUNTIME_DIR:-/tmp}/mm-council/<EX-or-uuid>-<iso8601-utc-Z>.md`. Verdict aggregation uses worst-of-N severity ladder (`proceed < proceed_with_changes < revise < reject`) matching the existing blocker rule; confidence is lowest among agreeing panelists, or `low` on disagreement. Filename uniqueness: EX ref if subject argument matched `(?i)EX(\d+)`, else UUID4. mm-council creates+owns the file; caller deletes after successful read; no auto-cleanup on caller failure (preserves synthesis for inspection). Marketplace description updated to mention `/review-ex`.
...
**Still open:** **Nothing.** All seven items (0/1.a/1.b/1.c/2/3/4/5/6) are shipped end-to-end across radbot PRs #113/#114/#115/#116 + claude-skills commits 62a7ddd/4938d34. The full council-loop is operational: Scout drafts plans → `/review-ex EX<N>` runs cross-family Opus+Gemini+GPT-5 review and flips `proposed → in_review → approved` → `/ship` flips `approved → executing → completed` and merges → `/postmortem-ex <PR#>` writes a JR with the `## Followups` markdown contract → "Scout, process postmortems" turns those into atomic dedup-keyed PT/EX/JR followups. The lifecycle ref-code flip from `EX_DRAFT_*` to a Telos `EX<N>` is the only remaining cosmetic — the spec body is 105KB, would dominate any PR diff, and on-disk markdown remains useful for git-diff-ing shipping status updates. Will be its own small operation when there's a clean reason to do it.

**Telemetry watch (one-week post-deploy, per Item 6 ship note):** the `context_injection` telemetry counter should drop noticeably for beto sessions (~75 tokens/turn anchor + ~500 tokens first-turn-block now retired) while scout-as-root totals stay flat. If beto's `transfer_to_agent` rate drops or "beto handles it himself" rate rises post-Item-6, revert is a single-line restore of `assembly.py:317` per Risks #5.
...
5. **Item 2 — ✅ SHIPPED via [claude-skills 62a7ddd](https://github.com/perrymanuk/claude-skills/commit/62a7ddd) (direct push to `claude-skills:main` 2026-05-03; that repo has no CI / no PR convention for skill-only changes).** Two artifacts in one commit: (a) `plugins/radbot/commands/review-ex.md` — new 7-step orchestration command. (b) `plugins/mm-council/commands/evaluate.md` step 6 — emits the `COUNCIL_VERDICT:` machine-readable line + writes synthesis to `${XDG_RUNTIME_DIR:-/tmp}/mm-council/<EX-or-uuid>-<iso8601-utc-Z>.md`. Verdict aggregation uses worst-of-N severity (`proceed < proceed_with_changes < revise < reject`); confidence is lowest-of-agreeing-panelists, or `low` on disagreement. Marker-balance validation runs both pre- and post-strip (closes Round 2 council "no change after sub" miss case). Single confirmation prompt for the `approved` flip on green verdicts; left at `in_review` on `revise`/`reject` — never rolled back to `proposed` on partial failure (review-attempt provenance stays visible). Marketplace description updated.
6. **Item 4 — ✅ SHIPPED via [claude-skills 4938d34](https://github.com/perrymanuk/claude-skills/commit/4938d34) (direct push to `claude-skills:main` 2026-05-03; skill-only convention).** New `plugins/radbot/commands/postmortem-ex.md` (365 LoC). 8-step orchestration: PR-URL canonicalization → resolve EX from branch/title/body with conflict abort → fetch+validate postmortem markers BEFORE journal_add (Round 2 step-ordering blocker resolved) → dedup query with `include_inactive` → generate `## Followups`-bearing markdown → `journal_add` → append `<!-- postmortem -->` block to EX body (cross-family block preserved) → optional `--supersede`/`--force-new` post-step → UX nudge. Both flag paths use load-bearing ordering (delayed-archive for supersede, mark-old-processed-FIRST for force-new) per Round 3 blocker resolutions. EX status preserved (single-ownership: /ship Phase 11 owns `executing → completed`).
7. **Item 5 — ✅ SHIPPED end-to-end (consumer side via PR #115's scout.md § 7; producer side via claude-skills 4938d34's `## Followups` markdown contract).** No additional artifacts needed — the followup conventions are encoded as runtime invariants in scout.md (consumer) + postmortem-ex.md (producer). Verification queries the spec described all work via Item 0.c's `metadata_filter` (PR #113).
